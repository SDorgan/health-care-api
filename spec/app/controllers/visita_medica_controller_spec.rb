...
describe 'VisitaMedicaController' do
  ...
  it 'deberia devolver la visita medica con la que se hizo POST' do # rubocop:disable RSpec/ExampleLength, Metrics/LineLength
    post '/visitas', { 'afiliado': @afiliado.id, 'prestacion': @prestacion.id, 'centro': @centro.id }.to_json, 'HTTP_API_KEY' => API_KEY
  ...
  it 'deberia devolver error si no se encuentra al afiliado' do
    post '/visitas', { 'afiliado': @afiliado.id + 1, 'prestacion': @prestacion.id, 'centro': @centro.id }.to_json, 'HTTP_API_KEY' => API_KEY

    expect(last_response.status).to be 401
    response = JSON.parse(last_response.body)
    expect(response['mensaje']).to eq 'El ID no pertenece a un afiliado'
  end

  it 'deberia devolver error si no se encuentra la prestacion' do
    post '/visitas', { 'afiliado': @afiliado.id, 'prestacion': @prestacion.id + 1, 'centro': @centro.id }.to_json, 'HTTP_API_KEY' => API_KEY
  ...
  it 'deberia devolver error si no se encuentra el centro' do
    post '/visitas', { 'afiliado': @afiliado.id, 'prestacion': @prestacion.id, 'centro': @centro.id + 1 }.to_json, 'HTTP_API_KEY' => API_KEY
  ...
  it 'deberia devolver error si la prestación no se da en el centro' do # rubocop:disable RSpec/ExampleLength, Metrics/LineLength
    otra_prestacion = Prestacion.new('Odontología', 600)
    otra_prestacion = PrestacionRepository.new.save(otra_prestacion)

    post '/visitas', { 'afiliado': @afiliado.id, 'prestacion': otra_prestacion.id, 'centro': @centro.id }.to_json, 'HTTP_API_KEY' => API_KEY

    expect(last_response.status).to be 404
    response = JSON.parse(last_response.body)
    expect(response['mensaje']).to eq 'La prestación pedida no se ofrece en el centro'
  end
