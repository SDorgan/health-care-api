...
Cuando('se registra caso sospechoso de COVID') do
  ...
  @response = Faraday.post(COVID_URL, @request.to_json, 'Content-Type' => 'application/json', 'HTTP_API_KEY' => API_KEY)
end
...
Entonces('queda registrado que el afiliado es sospechoso') do
  ...
  @response = Faraday.get(URL, @request.to_json, 'Content-Type' => 'application/json', 'HTTP_API_KEY' => API_KEY)
  json_response = JSON.parse(@response.body)
